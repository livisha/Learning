...
{
    public class QuickKartRepository:IRepository
    {
        ...
        public bool UpdateCategory(Categories category)
        {
            try
            {
                var catObjFromDB = context.Categories.Where(x => x.CategoryId == category.CategoryId).Select(x => x).FirstOrDefault();
                catObjFromDB.CategoryName = category.CategoryName;
                context.SaveChanges();
                return true;
            }
            catch (Exception)
        ...
        {
            string productId = context.Products.OrderByDescending(x => x.ProductId).Select(x => x.ProductId).FirstOrDefault().ToString();
